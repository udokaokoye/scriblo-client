...
function ProfileHeadCard({session, profile, follows, ServerdoesSignedInUserFollowProfile}) {
  const [doesSignedInUserFollowProfile, setdoesSignedInUserFollowProfile] = useState(ServerdoesSignedInUserFollowProfile)
    ...
    <div className="profileHeadCardContainer">
      <div className="profileCoverImage">
        ...
        {session?.email && session.id == profile?.id && (
            <div className="changeCoverInput" style={{display: 'none'}}>Change cover image</div>
        )}
      </div>
      <div style={{background: `url(${profile?.avatar})`}} className="profileAvatar"></div>
      <div className="profileInfo">
        <div className="profileNameAndFollowBtn">
          <h3>{profile?.name}</h3>
          {session?.id !== profile?.id ? (
            <div className="followAndEmailBtn">
            <button className="followBtn btn" onClick={() => followUserHandler()}>
        ...
        <div className="profileDateAndUrl">
          <span>
            {" "}
            <CalendarTodayOutlinedIcon className="profileIcon" /> Joined {formatDate(profile?.createdAt)}
          </span>
          {profile?.url ? (
            <span>
            ...
            <InsertLinkOutlinedIcon className="profileIcon" />{" "}
            {session?.id == profile?.id ? 'Add a link to your profile' : 'No link added'}
          </span>}
        </div>
