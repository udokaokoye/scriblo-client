    ...
    <div className={`profileContainer ${source_Sans_Pro.className}`}>
      ...
      <div className="followsContainer">
        <div className="followsHeader">
          <span className="headerText">Followers</span>
          <span className="closeFollows"><Link href={`/${user?.username}`}>x</Link></span>
        </div>
        <div className="followWrapper">
          {userFollows.followers.length > 0 &&
            userFollows.followers.map((follow) => (
              <>
              <FollowUserCard user={follow.user_details} profileId={user.id} userId={session.id} type={'follower'} />
                  </>
